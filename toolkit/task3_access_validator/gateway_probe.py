...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="Path to passord wordlist file.",
    )
...
def run_credentials_test(host, port, user, passwords, attemot_fn):
    total = len(passwords)
    for i, password in enumerate(passwords, start=1):
        print(f"[*] Attempt {i}/{total}: {user}:{password}")

        if attemot_fn(host, port, user, password):
            return password
...
def main():
    """Main orchestraation: parse args. load wordlist, run test, report."""
    ...
    if not passwords:
        print("[!] Wordlost is empy after cleaning .", file=sys.stderr)
        sys.exit(1)

    # Select teh attempt fucntion based on service
    if args.service == "ftp":
